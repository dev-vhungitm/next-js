...
const Header = () => {
  ...
  useEffect(() => {
    const getMenuContainerEls = () => {
      const menusEl = document?.querySelector('.' + styles.menus);
      const menuContainerEls = Object.values(menusEl.getElementsByClassName(styles.menu_container));

      return menuContainerEls;
    };

    const handleCloseSubMenusContainer = activeIndex => {
      const menuContainerEls = getMenuContainerEls();

      menuContainerEls.forEach((menuContainerEl, menuContainerElIndex) => {
        const subMenusContainerEl = menuContainerEl.querySelector('.' + styles.sub_menus_container);
    ...
    menuContainerEls.forEach((menuContainerEl, menuContainerElIndex) => {
      const menuSvg = menuContainerEl.querySelector('svg');
      const subMenusContainerEl = menuContainerEl.querySelector('.' + styles.sub_menus_container);

      if (subMenusContainerEl) {
        const subMenusEl = subMenusContainerEl.querySelector('.' + styles.sub_menus);
